...
let  TAG = "BeautyViewController"
    override func viewDidLoad() {
        super.viewDidLoad()
        self.view.backgroundColor = UIColor.grayColor()
        super.datas.addObjectsFromArray(BaseModelDB.getInstance().findAll(API.kBeautifulGirlUrlString))
        super.headerDatas.addObjectsFromArray(HeaderBaseModelDB.getInstance().findAll(API.kBeautifulGirlUrlString))
        super.tableView.reloadData()
        // Do any additional setup after loading the view.
    }
    override func getNetDatas(){
        //print("LastestViewController->getNetDatas")
        let urlStr =  String(format:  API.kBeautifulGirlUrlString, super.page)
        ...
        HTTPConnection.getRequestWithUrl(urlStr, param: nil) { (data, error) in
            ...
            if super.page == 1 {
                ...
                BaseModelDB.getInstance().deleteAll(API.kBeautifulGirlUrlString)
                HeaderBaseModelDB.getInstance().deleteAll(API.kBeautifulGirlUrlString)
            }
            //获取数据成功
            //result
            ...
            for dict in resultData {
                ...
                dict.setValue(API.kBeautifulGirlUrlString, forKey: "type")
