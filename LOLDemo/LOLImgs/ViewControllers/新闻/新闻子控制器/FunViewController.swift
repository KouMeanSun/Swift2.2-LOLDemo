...
let  TAG = "FunViewController"
    override func viewDidLoad() {
        super.viewDidLoad()
        self.view.backgroundColor = UIColor.brownColor()
        super.datas.addObjectsFromArray(BaseModelDB.getInstance().findAll(API.kGreatUrlString))
        super.headerDatas.addObjectsFromArray(HeaderBaseModelDB.getInstance().findAll(API.kGreatUrlString))
        super.tableView.reloadData()
        // Do any additional setup after loading the view.
    }
    override func getNetDatas(){
        let urlStr =  String(format:  API.kGreatUrlString, super.page)
        ...
        HTTPConnection.getRequestWithUrl(urlStr, param: nil) { (data, error) in
            ...
            if super.page == 1 {
                ...
                BaseModelDB.getInstance().deleteAll(API.kGreatUrlString)
                HeaderBaseModelDB.getInstance().deleteAll(API.kGreatUrlString)
            }
            //获取数据成功
            //result
            ...
            for dict in resultData {
                ...
                dict.setValue(API.kGreatUrlString, forKey: "type")
